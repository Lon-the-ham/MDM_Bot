        ...
        while True:
            try:
                ...
                if str(reaction.emoji) == "ℹ️" and cur_page != 1:
                    ...
                    cur_page = 3
                    new_embed = discord.Embed(title = band_name, description="", url=band_link, color=0x1b0000)
                    new_embed.set_thumbnail(url=picture_url)
                    new_embed.add_field(name="Line-up:", value=curlineup_msg, inline=False)
                    await message.edit(embed=new_embed)
                    await message.remove_reaction(reaction, user)
                    ...
                    new_embed = discord.Embed(title = band_name, description="", url=band_link, color=0x1b0000)
                    new_embed.set_thumbnail(url=picture_url)
                    new_embed.add_field(name="Similar bands:", value=similar_msg, inline=False)
                    await message.edit(embed=new_embed)
                    await message.remove_reaction(reaction, user)
